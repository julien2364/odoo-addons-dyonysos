...
from odoo.addons.dyo_mcp_server.models.mcp_model_config import CREATING_METHODS
...
class OdooMcpController(http.Controller):
    ...
    def mcp_endpoint(self, **kwargs):
        ...
        if icp.get_param("dyo_mcp_server.enabled", "True") != "True":
    ...
    def _check_rate_limit(uid):
        limit = int(request.env["ir.config_parameter"].sudo().get_param("dyo_mcp_server.rate_limit", "120") or 120)
        if limit <= 0:
            return True
    ...
    def _writes_allowed(self):
        return request.env["ir.config_parameter"].sudo().get_param("dyo_mcp_server.allow_writes", "False") == "True"
    ...
    def _on_tools_call(self, params):
        ...
        icp = request.env["ir.config_parameter"].sudo()
        if icp.get_param("dyo_mcp_server.log_arguments", "True") == "True":
