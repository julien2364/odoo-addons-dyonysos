...
class TestMcpServer(HttpCase):
    ...
    def setUpClass(cls):
        ...
        cls.icp.set_param("dyo_mcp_server.enabled", "True")
        cls.icp.set_param("dyo_mcp_server.allow_writes", "False")
        cls.icp.set_param("dyo_mcp_server.rate_limit", "0")
        cls.mcp_user = cls.env["res.users"].create({
            "name": "MCP Bot", "login": "mcp-bot",
            "group_ids": [(6, 0, [
                cls.env.ref("base.group_user").id,
                cls.env.ref("dyo_mcp_server.group_mcp_user").id,
    ...
    def test_04_tools_list_hides_writes_when_disabled(self):
        ...
        self.icp.set_param("dyo_mcp_server.allow_writes", "True")
        try:
            names = [t["name"] for t in self._rpc("tools/list").json()["result"]["tools"]]
            self.assertIn("odoo_create", names)
            self.assertIn("odoo_call_method", names)
        finally:
            self.icp.set_param("dyo_mcp_server.allow_writes", "False")
    ...
    def test_13_endpoint_can_be_switched_off(self):
        self.icp.set_param("dyo_mcp_server.enabled", "False")
        try:
            self.assertEqual(self._rpc("tools/list").status_code, 503)
        finally:
            self.icp.set_param("dyo_mcp_server.enabled", "True")
